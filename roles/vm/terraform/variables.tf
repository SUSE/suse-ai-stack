...
variable "airgap_services" {
  description = "Optional CPU-only services node used by the AI Factory air-gap lab"
  type = object({
    enabled          = optional(bool, false)
    instance_type    = optional(string, "m6i.xlarge")
    root_volume_size = optional(number, 120)
    controller_cidr  = optional(string, "127.0.0.1/32")
  })
  default = {}

  validation {
    condition     = !try(var.airgap_services.enabled, false) || can(cidrhost(var.airgap_services.controller_cidr, 0))
    error_message = "airgap_services.controller_cidr must be a valid CIDR when the services node is enabled."
  }
}
