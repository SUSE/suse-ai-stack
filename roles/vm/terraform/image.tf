...
data "aws_ami" "aws_ami_sle_micro_suse_ai" {
  #count = var.suse_ai_cluster["image_distro"] == "sle-micro" ? 1 : 0
...
data "aws_ami" "aws_ami_sles_suse_ai" {
  #count = var.suse_ai_cluster["image_distro"] == "sles" ? 1 : 0
...
data "aws_ami" "aws_ami_sle_micro_suse_observability_cluster" {
  #count = var.suse_observability_cluster["image_distro"] == "sle-micro" ? 1 : 0
...
data "aws_ami" "aws_ami_sles_suse_observability_cluster" {
  #count = var.suse_observability_cluster["image_distro"] == "sles" ? 1 : 0
