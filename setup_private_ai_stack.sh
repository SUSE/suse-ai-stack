...
export ANSIBLE_ROLES_PATH=${PROJECT_DIR}/roles:${PROJECT_DIR}/external_playbooks/roles
...
### CLONE rke2-rancher-ansible.git repo as external_playbooks
EXT_REPO_URL="https://github.com/mbelur/rke2-rancher-ansible.git"
EXT_REPO_BRANCH="main"
DEST_DIR="${PROJECT_DIR}/external_playbooks"
if [ -d "$DEST_DIR/.git" ]; then
    echo "Updating existing repo at $DEST_DIR..."
    git -C "$DEST_DIR" fetch --depth 1 origin $EXT_REPO_BRANCH
    git -C "$DEST_DIR" reset --hard origin/$EXT_REPO_BRANCH
else
    echo "Cloning repo into $DEST_DIR..."
    git clone --depth 1 --branch "$EXT_REPO_BRANCH" "$EXT_REPO_URL" "$DEST_DIR"
fi
...
### DEPLOY nvidia-driver-installer
