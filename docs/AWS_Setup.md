...
By default, we create a management RKE2 cluster where all the services including the AI workloads are deployed and suse observability components are deployed when enable_suse_observability is true.
...
We support deploying into SLE-Micro and SLES based EC2 instances.
...
6. Update your local `/etc/hosts` file with the public IP of your instance based on the instructions displayed when the `setup_private_ai_stack.sh` completes.
   For example:
   ```console
   34.214.10.56 mgmt-rancher suse-ollama-webui rancher.demo longhorn suse-observability
   ```
...
6. Point your browser to `https://<open_webui_hostname>` to access the WebUI,
   ...
   Rancher is accessible by pointing your browser to https://rancher.demo, using
